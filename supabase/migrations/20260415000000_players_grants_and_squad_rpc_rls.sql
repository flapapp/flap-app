-- PostgREST calls RPCs as `authenticated`; nested INSERT/UPDATE can still hit RLS.
-- Ensure API role can use `players`, and let the squad RPC apply club + roster atomically.

grant select, insert, update, delete on table public.players to authenticated;
grant all on table public.players to service_role;

-- Optional: sequences if any (none for uuid default)
...
begin
  ...
  -- Bypass RLS for rows this RPC owns (caller is captain; avoids 403 under PostgREST).
